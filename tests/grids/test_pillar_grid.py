...
def test_from_rectilinear_constructs_expected_shapes(rectilinear_vertices):
    ...
    grid = PillarGrid.from_rectilinear(x=x, y=y, z_top=1000.0, z_bottom=1100.0)
...
def test_from_rectilinear_rejects_non_monotonic_x():
    ...
    with pytest.raises(ValueError, match="strictly increasing"):
        PillarGrid.from_rectilinear(x=x, y=y, z_top=0.0, z_bottom=1.0)
...
def test_from_rectilinear_rejects_invalid_z_order():
    ...
    with pytest.raises(ValueError, match="z_bottom"):
        PillarGrid.from_rectilinear(x=x, y=y, z_top=10.0, z_bottom=10.0)


def test_from_regular_with_step_sizes_creates_expected_extents():
    grid = PillarGrid.from_regular(xlim=(0.0, 100.0), ylim=(0.0, 80.0), dx=25.0, dy=20.0, z_top=0.0, z_bottom=10.0)

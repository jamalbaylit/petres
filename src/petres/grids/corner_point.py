...
class CornerPointGrid:
    ...
    Attributes:
        ...
        corner_z (np.ndarray): Z-coordinates at cell corners, shape (nk, nj, ni, 2, 2, 2)
    ...
    Example:
        ...
        ... )
        >>> corner_z = ...  # Define corner depths
        >>> grid = CornerPointGrid(pillars, corner_z)
        >>> print(grid.shape)  # (10, 10, 5)
    """
    
    pillars: PillarGrid
    corner_z: np.ndarray                     # Shape (nk, nj, ni, 2, 2, 2)
    ...
    def __post_init__(self):
        """Validate dimensions."""
        ni, nj = self.pillars.cell_shape
        nk = self.corner_z.shape[0]
        
        expected_corner_shape = (nk, nj, ni, 2, 2, 2)
        if self.corner_z.shape != expected_corner_shape:
            raise ValueError(
                f"corner_z shape {self.corner_z.shape} != expected {expected_corner_shape}"
            )
    ...
    def get_cell_corners(self, k: int, j: int, i: int) -> np.ndarray:
        ...
        for idx, (di, dj) in enumerate([(0,0), (1,0), (1,1), (0,1)]):
            z = self.corner_z[k, j, i, di, dj, 0]
            corners[idx] = self.pillars.interpolate_at_z(j+dj, i+di, z)
        
        # Top face (dk=1)
        for idx, (di, dj) in enumerate([(0,0), (1,0), (1,1), (0,1)]):
            z = self.corner_z[k, j, i, di, dj, 1]
    ...
    def from_layers(
        ...
        # Build corner_z from layer interfaces
        corner_z = np.zeros((nk, nj, ni, 2, 2, 2))
        
        for k in range(nk):
            for j in range(nj):
                for i in range(ni):
                    # Bottom (dk=0) - all 4 corners same z
                    corner_z[k, j, i, :, :, 0] = layer_z[k, j, i]
                    # Top (dk=1) - all 4 corners same z
                    corner_z[k, j, i, :, :, 1] = layer_z[k+1, j, i]
        
        # Keep active as None if not provided (memory efficient)
        if properties is None:
            properties = {}
        
        return cls(pillars, corner_z, active, properties)

...
#         # Detect if this zone shares a boundary with the previous zone
#         # Skip first level (t=0.0) ONLY if zones share the boundary
#         if iz > 0:
#             prev_base = Z_interfaces[-1]  # Last interface added (previous zone's base)
            
#             # Check if top of current zone matches base of previous zone
#             # Use small tolerance for floating point comparison
#             tolerance = 1e-6
#             zones_are_continuous = np.allclose(zt, prev_base, atol=tolerance, rtol=0)
            
#             if zones_are_continuous:
#                 # Shared boundary: skip first level to avoid duplicate
#                 levels = levels[1:]
#             # else: gap exists, keep all levels including first (top)
